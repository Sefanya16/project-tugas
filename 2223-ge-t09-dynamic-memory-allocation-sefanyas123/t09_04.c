...
int main(int _argc, char **_argv) 
{
  ...
  struct course_t *courses = malloc(12 * sizeof(struct course_t));
  ...
  do
    {
        ...
        else if (strstr(data, "create-course") != NULL)
        {
          ...
          courses[j] = create_course(code, name, credit, gradef);
          j++;
        }
        ...
        else if (strstr(data, "print-courses") != NULL) 
        {
         ...
         else if (strstr(data, "find-student-by-id") != NULL)
        {
        kata = strtok(data, "#");
         kata = strtok(NULL, "#");
         strcpy(id, kata);
         ...
         else if (strstr(data, "find-course-by-code") != NULL)
        {
        kata = strtok(data, "#");
         kata = strtok(NULL, "#");
         strcpy(code, kata);
           for (int m = 0; m < i; m++)
         {
         if (strcmp(courses[m].code, code) == 0)
         {
            print_course(courses[m]);
            break;
            }
        }
        }
